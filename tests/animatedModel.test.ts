...
const MODEL_PATH = fileURLToPath(
  new URL("../public/models/sword-machine.glb", import.meta.url),
);
...
const TREANT_PATH = fileURLToPath(
  new URL("../public/models/treant.glb", import.meta.url),
);
const STATIC_MESH_PATH = fileURLToPath(
  new URL("../public/models/slime.glb", import.meta.url),
);
...
describe("animated bounds sampling order", () => {
  it("matches an indexed mesh to its draw-order-expanded equivalent", () => {
    ...
    expanded.setAttribute(
      "skinIndex",
      new THREE.BufferAttribute(expandedSkinIndices, 4),
    );
    expanded.setAttribute(
      "skinWeight",
      new THREE.BufferAttribute(expandedSkinWeights, 4),
    );
    ...
    expect(indexedBounds.bounds.min.toArray()).toEqual(
      expandedBounds.bounds.min.toArray(),
    );
    expect(indexedBounds.bounds.max.toArray()).toEqual(
      expandedBounds.bounds.max.toArray(),
    );
...
describe("instanced animation shader", () => {
  it("splices baked skinning into both normal and position phases", () => {
    ...
    try {
      ...
      const normalStart = shader.vertexShader.indexOf(
        "#include <beginnormal_vertex>",
      );
      const skinNormal = shader.vertexShader.indexOf("mat4 bakedSkinMatrix");
      const positionStart = shader.vertexShader.indexOf(
        "#include <begin_vertex>",
      );
      const skinPosition = shader.vertexShader.indexOf(
...
describe("direct Unity-sampled FireDragon", () => {
  it("loads all 129 poses as a compact standard skeletal model", () => {
    ...
    expect(fireDragonAll.geometry.morphAttributes.position ?? []).toHaveLength(
      0,
    );
    expect(fireDragonAll.geometry.morphAttributes.normal ?? []).toHaveLength(0);
    expect(
      Object.fromEntries(
        [...fireDragonAll.clips].map(([name, clip]) => [name, clip.frameCount]),
      ),
    ...
    expect(
      Array.from(fireDragonAll.boneTexture.image.data as Float32Array).every(
        Number.isFinite,
      ),
    ).toBe(true);
  });
  ...
  it("filters the gallery load to run while keeping exact skeletal bounds", () => {
    ...
    expect(fireDragonRun.bindMatrixInverse.equals(new THREE.Matrix4())).toBe(
      true,
    );
    ...
    expect(fireDragonRun.geometry.getAttribute("position").getX(0)).toBeCloseTo(
      31.1666718,
      6,
    );
  });
...
describe("animated mesh-node transforms", () => {
  it("matches stock SkinnedMesh output when an animated joint owns the mesh", async () => {
    ...
    try {
      ...
      expect(
        matrixMaxDelta(loaded.restNode, loaded.mesh.matrixWorld),
      ).toBeGreaterThan(1);
...
describe("authored bounds prefixes", () => {
  it("frames Treant by its body without removing the attack Log", async () => {
    ...
    try {
      ...
      const runLogSize = bakedVertexBounds(run, 6060, 6240, 0).getSize(
        new THREE.Vector3(),
      );
      ...
      expect(
        Math.min(attackLogSize.x, attackLogSize.y, attackLogSize.z),
      ).toBeGreaterThan(20);
    } finally {
...
async function loadStockAndBaked(
  ...
  if (frame >= bakedClip.frameCount)
    throw new Error("sample frame is out of range");
...
function skinVertex(
  ...
  for (let component = 0; component < 4; component++) {
    ...
    bone.fromArray(
      matrices,
      matrixOffset + skinIndex.getComponent(vertex, component) * 16,
    );
