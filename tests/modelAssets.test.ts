...
describe("Athena2 authored model catalog", () => {
  it("contains unique complete entries", () => {
    ...
    expect(new Set(catalog.models.map((model) => model.unit)).size).toBe(
      catalog.models.length,
    );
    expect(new Set(catalog.models.map((model) => model.file)).size).toBe(
      catalog.models.length,
    );
    for (const model of catalog.models) {
      ...
      expect(Object.keys(model).sort(), model.unit).toEqual(
        expectedKeys.sort(),
      );
      ...
      for (const name of ["run", "attack", "die"] as const) {
        ...
        expect(clip.duration, `${model.unit} ${name} duration`).toBeGreaterThan(
          0,
        );
        expect(clip.static, `${model.unit} ${name} must be animated`).toBe(
          false,
        );
        expect(
          clip.frames,
          `${model.unit} ${name} baked frames`,
        ).toBeGreaterThan(1);
        expect(
          clip.frameRate,
          `${model.unit} ${name} frame rate`,
        ).toBeGreaterThan(0);
  ...
  it("ships no model or skin outside the complete catalog", async () => {
    ...
    const actual = files
      .filter((file) => file.endsWith(".glb") || file.endsWith(".ktx2"))
      .sort();
    const expected = catalog.models
      .flatMap((model) => [model.file, ...model.skins])
      .sort();
  ...
  it("keeps Treant's attack prop while binding clips to its skin joints", async () => {
    const model = catalog.models.find(
      (candidate) => candidate.unit === "Treant",
    );
    ...
    expect(
      positionAccessor === undefined
        ? undefined
        : json.accessors?.[positionAccessor]?.count,
      "Treant full body-and-Log vertex count",
    ).toBe(6240);
    const meshNode = json.nodes?.find((node) => node.mesh === 0);
    expect(
      meshNode?.extras?.boundsVertexCount,
      "Treant body-only bounds prefix",
    ).toBe(6060);
    const joints = new Set(json.skins?.[0]?.joints ?? []);
    const jointNames = new Set(
      [...joints].map((index) => json.nodes?.[index]?.name),
    );
    ...
    const animatedJoints = new Set(
      (run?.channels ?? [])
        ...
        .filter(
          (node): node is number => node !== undefined && joints.has(node),
        )
  ...
  it("ships FireDragon as a compact Unity-sampled skeletal GLB", async () => {
    const model = catalog.models.find(
      (candidate) => candidate.unit === "FireDragon",
    );
    expect(model).toBeDefined();
    const glb = await readFile(join(MODEL_ROOT, model!.file));
    expect(
      glb.byteLength,
      "FireDragon compact skeletal size",
    ).toBeLessThanOrEqual(650_000);
    ...
    expect(
      primitive?.targets,
      "FireDragon has no morph targets",
    ).toBeUndefined();
    expect(
      json.skins?.[0]?.joints,
      "FireDragon sampled skin joints",
    ).toHaveLength(66);
    ...
    for (const [name, keys, channels] of [
      ...
      const animation = json.animations?.find(
        (candidate) => candidate.name === name,
      );
      expect(
        animation?.channels,
        `FireDragon ${name} skeletal channels`,
      ).toHaveLength(channels);
      expect(
        new Set(animation?.channels?.map((channel) => channel.target?.path)),
      ).toEqual(new Set(["rotation", "translation"]));
  ...
  it.each(catalog.models)(
    ...
    async (model) => {
      ...
      for (const animation of animations) {
        ...
        expect(
          duration,
          `${model.unit} ${animation.name} exported duration`,
        ).toBeCloseTo(model.clips[name].duration, 5);
      }
    },
  );
  ...
  it.each(catalog.models)(
    ...
    async (model) => {
      const [blue, red] = await Promise.all(
        model.skins.map(async (skin) =>
          ktx2Info(await readFile(join(MODEL_ROOT, skin))),
        ),
      );
...
function ktx2Info(buffer: Buffer): Ktx2Info {
  expect(buffer.subarray(0, KTX2_IDENTIFIER.length), "KTX2 identifier").toEqual(
    KTX2_IDENTIFIER,
  );
