 ...
 * exactly one free supply a Burstbot still pops out every time while a Slicebot
...
describe('training', () => {
  it('delivers every unit type when supply is plentiful', () => {
    for (const unit of [EntityType.Burstbot, EntityType.Slicebot, EntityType.Firespout]) {
  ...
  it('releases the held unit as soon as the supply arrives', () => {
    const { sim, barracks } = withBarracks();
    train(sim, barracks, EntityType.Slicebot);
    runPinned(sim, defOf(EntityType.Slicebot).buildTicks + 100, 1);
    expect(countOf(sim, EntityType.Slicebot)).toBe(0);

    // A depot finishes: the wait was the only thing holding it.
    sim.world.players[0]!.supplyMax = sim.world.players[0]!.supplyUsed + 10;
    sim.step([]);
    expect(countOf(sim, EntityType.Slicebot)).toBe(1);
  });
  ...
  it('refunds a cancelled unit in full, even one already finished', () => {
    const { sim, barracks } = withBarracks();
    const cost = defOf(EntityType.Slicebot).mineralCost;
    const before = sim.world.players[0]!.minerals;

    train(sim, barracks, EntityType.Slicebot);
    expect(sim.world.players[0]!.minerals).toBe(before - cost);

    runPinned(sim, defOf(EntityType.Slicebot).buildTicks + 60, 1);
    expect(sim.world.pool.prodCount[barracks]).toBe(1);
